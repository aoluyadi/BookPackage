...
Then, use the `book_details()` to retrieve information about a particular book and their ranking within a specific category in good reads. Here lets retrieve the Best Historical Fiction.

```{r}
book_details("https://www.goodreads.com/list/show/43.Best_Young_Adult_Books")
```

The object that is returned is book info date set: book name, rank, author, cover, and ranking chunk. This provides you with the rank information that is then used directly as an argument for book_cover. `book_cover()` allows you to view the book cover images. Here is the code to load the cover for Harry Potter and Sorcerer’s Stone (Harry Potter, #1). 
